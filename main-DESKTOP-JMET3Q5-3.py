...
if __name__ == '__main__':
    ...
    sell_price = float('{:.4f}'.format(buy_price * (1.02)))
        ...
        if price <= buy_price:
            ...
            for wallet in coinbase_request('GET', '/api/v3/brokerage/accounts?limit=250', '')['accounts']:
                if wallet['currency'] == cur and float(wallet['available_balance']['value']) >= min_currency_bal:
                    ...
                    #     fiat_cur = '{:.floored_decf}'.format(math.floor((float(set_usd_accnt_bal/ price) -
                    #                                ((set_usd_accnt_bal / price) * 0.0025))))
                    # else:
                    #     break
                    # min_asset_bal = float(fiat_cur)
                    # For Set Amount Of Cryptocurrency That Allows Decimal Sizes
                    # fiat_cur = '{:.4f}'.format((float(set_usd_accnt_bal/ price) -
                    #                                ((set_usd_accnt_bal / price) * 0.0025)))
                    # min_asset_bal = float(fiat_cur)
                    ...
                    # fiat_cur = '{:.4f}'.format(math.floor(float(wallet['available_balance']['value'])))
                    # min_asset_bal = '{:.4f}'.format((float(wallet['available_balance']['value']) / price) -
                    #                                ((float(wallet['available_balance']['value']) / price) * 0.0025))
                    # # print(fiat_cur)
                    print(cur + ' Available: ' + '{:.2f}'.format(float(wallet['available_balance']['value'])))
                    # Limit Buy Order (Max)
                    if coin in spcl_assts:
                        last_buy_ordr = placeLimitOrder(Side.BUY.name, asset_id, fiat_cur,  '{:.4f}'.format(float(price)))
                    else:
            ...
            if price > sell_price:
                buy_price = float('{:.4f}'.format(price - (price * 0.02)))
                sell_price = float('{:.4f}'.format(buy_price * (1.02)))
            ...
            for asset in coinbase_request('GET', '/api/v3/brokerage/accounts?limit=250', '')['accounts']:
                if asset['currency'] == coin and float(asset['available_balance']['value']) >= min_asset_bal:
                    ...
                    # For Cryptocurrency That Allows Decimal Sizes
                    coin_cur = '{:.1f}'.format((float(asset['available_balance']['value'])) -
                                               (float(asset['available_balance']['value']) * 0.0025))
                    ...
                    if asset['currency'] == cur and float(asset['available_balance']['value']) >= min_currency_bal:
                        ...
                        print(cur + ' Available: ' + '{:.4f}'.format(float(asset['available_balance']['value'])))
                        print(f'Buy price: ${str(buy_price)}')
                        print(f'Sell price: ${str(sell_price)}')
                        if delay == 1:
                            print(f'Checking again in {str(delay)} second...')
    ...
    #                 fiat_cur = '{:.4f}'.format((float(wallet['available_balance']['value']) / price) -
    #                                            ((float(wallet['available_balance']['value']) / price) * 0.0025))
    #                 print(wallet)
    #                 # sb_lmtbuy_asst_amnt = '{:.4f}'.format(math.floor(float(fiat_cur)/sb_buy_price))
    #                 # print('Currency balance converted to cryptocurrency: ' + sb_lmtbuy_asst_amnt)
    #                 print(sb_cur + ' Available: ' + '{:.4f}'.format(float(wallet['available_balance']['value'])))
    ...
    #                         print(sb_coin + ' Available: ' + '{:.4f}'.format(float(wallet['available_balance']['value'])))
    #                         if sb_delay == 1:
    #                             print(f'Checking again in {str(sb_delay)} second...')
    #                         else:
    #                             print(f'Checking again in {str(sb_delay)} seconds...')
    ...
    #                 # '{:.4f}'.format(math.floor(float(asset['available_balance']['value'])))
    #                 # '{:.4f}'.format(float(asset['available_balance']['value']))
    ...
    #                     print('You\'ve already made a profit\n'
    #                           'and are waiting to buy back in')
    #                     print(sb_cur + ' Available: ' + '{:.4f}'.format(float(asset['available_balance']['value'])))
    #                     if sb_delay == 1:
    #                         print(f'Checking again in {str(sb_delay)} second...')
    #                     else:
    #                         print(f'Checking again in {str(sb_delay)} seconds...')
