...
if __name__ == '__main__':
    ...
    cur = 'USD'
    txt_filename = 'bot_cmd_line_setup.txt'
    ...
    price_dec_plcs = int(asset_args[2])
    amnt_dec_plcs = int(asset_args[3])
    ...
    orig_buy_price = float(('{:.' + str(price_dec_plcs) + 'f}').format(float(client.get_product(asset_id).to_dict()['price'])))
    ...
    maker_fee = 0.0015
    fee = 0.0025
    min_currency_bal = float(sys.argv[5])
    ...
    select_limit_buy_order = sys.argv[8].lower()
    select_limit_sell_order = sys.argv[9].lower()

    size = 0.0

    # delay_list = [1, 5, 10, 15, 20, 30, 60, 120, 150, 300, 600, 900, 1800, 3600]
    delay = float(sys.argv[10])
    count = 0 # float(sys.argv[11])
    fee = 0.0025
    last_buy_ordr = {}
    last_sell_ordr = {}
    flld_buy_ordr = {}
    ...
    while True:
        ...
        file_path = f'flld_buy_ordrs_{asset_id}_main4[{dt.now().strftime("%b_%d_%Y")}]{unique_identifier}.txt'
        ...
        if price <= buy_price:
            ...
            if float(wallet['available_balance']['value']) >= min_currency_bal:
                ...
                if float(asst['available_balance']['value']) >= size and count == 0:
                    ...
                    print(coin + ' Available: ' + ('{:.' + str(amnt_dec_plcs) + 'f}').format(float(asst['available_balance']['value'])))   
                if float(wallet['available_balance']['value']) < min_currency_bal:
                    print('---------------------------------------------\n')
                    print('Your account balance is below the minumum required to trade.')
                    print(f'Minimum amount required: ${str(min_currency_bal)}')
                    print(f"Current account balance: ${'{:.2f}'.format(float(wallet['available_balance']['value']))}")
                    print('\n---------------------------------------------\n')
            ...
            if len(filled_buy_order_list) >=1:
                ...
                if sell_order_type == 'ONE':
                    ...
                    print(f"Selling {('{:.' + str(amnt_dec_plcs) + 'f}').format(float(asset['available_balance']['value']))} of {coin}!")
